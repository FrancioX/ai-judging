"""Sweep of_drift_guard_px on the 4-video mini set.

Tests values 50, 100, 150, 200 (current) — Exp 20→21a showed reducing
drift guard from 0 to 200 improved Arno by 3.5px; check tighter values.

All other params from Exp 21a config.

Usage (from worktree root):
    uv run python scripts/sweep_drift_guard.py
"""
...
RESULTS_PATH = Path("output/sweep_drift_guard_results.json")
...
def _run_tracking(video_stem: str, of_drift_guard_px: float) -> int:
    """Run tracking with the given drift guard value; return n_frames."""
    ...
    track_skier(
        ...
        of_drift_guard_px=of_drift_guard_px,  # <-- sweep param
...
def main() -> None:
    ...
    for val in SWEEP_VALUES:
        label = str(val)
        print(f"\n{'=' * 60}")
        print(f"  of_drift_guard_px = {val}  (current best = 200)")
        print(f"{'=' * 60}")
        ...
        for stem, short in VIDEOS:
            print(f"\n  --- {short} ---")
            n = _run_tracking(stem, float(val))
            total_frames += n
    ...
    print(f"{'=' * 80}")
    print(
        f"{'drift_guard':>11} | {'Arno':>8} | {'Andreas':>8} | "
    ...
    for val in SWEEP_VALUES:
        ...
        marker = " *" if val == 200 else "  "
        print(
            f"{val:>11}{marker}| {errs[0]:>8.1f} | {errs[1]:>8.1f} | "
