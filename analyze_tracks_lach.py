#!/usr/bin/env python3
"""Analyze track lengths from segmentation data - Lach Powell video."""
import json
...
if short_tracks:
    ...
    total_all = sum(l for _, l in sorted_tracks)
    print(f'Summary: {len(short_tracks)} tracks would be ignored with min_track_frames=10')
    print(f'These represent {total_short} detection frames ({(total_short/total_all*100):.2f}% of all detections)')
else:
    print('Summary: No tracks shorter than 10 frames')
